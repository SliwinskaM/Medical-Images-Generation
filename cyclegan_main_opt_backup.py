...
def train_cyclegan(models,
    ...
    """
    # open hdf5 file
    diskfile = h5py.File(hdf5_filename, "r+") # Read/write, file must exist
    ...
    # train dataset
    source_data, target_data, test_source_data, test_target_data = diskfile['source_data'], diskfile['target_data'], \
                                                                   diskfile['test_source_data'], diskfile['test_target_data']
    print("1")
    ...
    if patch > 1:
        ...
        valid = diskfile.create_dataset('valid', data=np.ones((batch_size,) + d_patch))
        fake = diskfile.create_dataset('fake', data=np.zeros((batch_size,) + d_patch))
    else:
        valid = diskfile.create_dataset('valid', data=np.ones([batch_size, 1]))
        fake = diskfile.create_dataset('fake', data=np.zeros([batch_size, 1]))

    print("2")
    valid_fake = diskfile.create_dataset('valid_fake', data=np.concatenate((valid, fake)))
    print("3")
    ...
    for step in range(train_steps):
        ...
        real_target = diskfile.create_dataset('real_target', data=target_data[rand_indexes])
        print("4")
        ...
        rand_indexes = np.sort(rand_indexes)
        # add real source samples to hdf5 file
        real_source = diskfile.create_dataset('real_source', data=source_data[rand_indexes])
        print("5")



        # generate a batch of fake target data fr real source data
        fake_target = diskfile.create_dataset('fake_target', data=g_target.predict(real_source))
        print("6")

        # combine real and fake into one batch
        x = diskfile.create_dataset('x', data=np.concatenate((real_target, fake_target)))
        print("7")
        ...
        fake_source = diskfile.create_dataset('fake_source', data=g_source.predict(real_target))
        print("9")
        x = diskfile.create_dataset('x', data= np.concatenate((real_source, fake_source)))
        print("10")
        ...
        del diskfile['real_target']
        del diskfile['real_source']
        del diskfile['fake_target']
        del diskfile['fake_source']

    # save the models after training the generators
    g_source.save(model_name + "-g_source.h5")
    g_target.save(model_name + "-g_target.h5")

    del diskfile['valid_fake']
    del diskfile['valid']
    del diskfile['fake']
...
def main(source_name, target_name, num_of_data=20, g_models=None):
    ...
    shapes, hdf5_filename = other_utils_opt.load_data(source_name, target_name, num_of_data=num_of_data)
    ...
    diskfile = h5py.File(hdf5_filename, "r")
