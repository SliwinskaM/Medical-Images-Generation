...
def ct_cross_mr(source_name, target_name, g_models=None):
    """Build and train a CycleGAN that can do
            CT <--> different variants of MR images
        """
    ...
    data, shapes = CT_MR_utils.load_data(source_name, target_name)
...
def t1_cross_stir(source_name, target_name, g_models=None):
    """Build and train a CycleGAN that can do
            T1 <--> STIR
        """
    ...
    data, shapes = T1_STIR_utils.load_data(source_name, target_name)
    source_data, _, test_source_data, test_target_data = data
    titles = (source_name + ' predicted source images.',
              target_name + ' predicted target images.',
              source_name + ' reconstructed source images.',
...
if __name__ == '__main__':
    ...
    if args.data_type == 'ct_mr':
        ...
        ct_cross_mr(source_name=args.source_name, target_name=args.target_name)
    if args.data_type == 't1_stir':
        print(gpu_device_name())
        t1_cross_stir(source_name=args.source_name, target_name=args.target_name)
